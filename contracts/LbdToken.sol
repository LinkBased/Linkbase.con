...
/**
 * @title LBD ERC20 token
 * @dev This is part of an implementation of the LBD Index Fund protocol.
 *      LBD is a normal ERC20 token, but its supply can be adjusted by splitting and
 *      combining tokens proportionally across all wallets.
 *
 *      LBD balances are internally represented with a hidden denomination, 'shares'.
 *      We support splitting the currency in expansion and combining the currency on contraction by
 *      changing the exchange rate between the hidden 'shares' and the public 'LBD'.
 */
contract LbdToken is ERC20UpgradeSafe, ERC677Token, OwnableUpgradeSafe {
    // PLEASE READ BEFORE CHANGING ANY ACCOUNTING OR MATH
    // Anytime there is division, there is a risk of numerical instability from rounding errors. In
    // order to minimize this risk, we adhere to the following guidelines:
    // 1) The conversion rate adopted is the number of shares that equals 1 LBD.
    //    The inverse rate must not be used--totalShares is always the numerator and _totalSupply is
    //    always the denominator. (i.e. If you want to convert shares to LBD instead of
    //    multiplying by the inverse rate, you should divide by the normal rate)
    // 2) Share balances converted into LbdToken are always rounded down (truncated).
    //
    // We make the following guarantees:
    // - If address 'A' transfers x LbdToken to address 'B'. A's resulting external balance will
    //   be decreased by precisely x LbdToken, and B's external balance will be precisely
    //   increased by x LbdToken.
    //
    ...
    uint256 private constant INITIAL_SUPPLY = 100670 * 10**DECIMALS;
    ...
    // This is denominated in LbdToken, because the shares-LBD conversion might change before
    // it's fully paid.
    mapping (address => mapping (address => uint256)) private _allowedLBD;
    ...
    /**
     * @dev Notifies LbdToken contract about a new rebase cycle.
     * @param supplyDelta The number of new LBD tokens to add into circulation via expansion.
     * @return The total number of LBD after the supply adjustment.
     */
        ...
        returns (uint256)
    {
        ...
        _sharesPerLBD = _totalShares.div(_totalSupply);

        // From this point forward, _sharesPerLBD is taken as the source of truth.
        // We recalculate a new _totalSupply to be in agreement with the _sharesPerLBD
        // conversion rate.
        ...
        initializer
    {
        ...
        BIG_TRANSFER_PERCENTAGE = 250;
        SMALL_TRANSFER_PERCENTAGE = 50;
    ...
    /**
     * @return The total number of LBD.
     */
    ...
    function balanceOf(address who)
        ...
        returns (uint256)
    {
        return _shareBalances[who].div(_sharesPerLBD);
    }
        ...
        returns (bool)
    {
        ...
        uint256 shareValue = value.mul(_sharesPerLBD);
    ...
    function allowance(address owner_, address spender)
        ...
        returns (uint256)
    {
        return _allowedLBD[owner_][spender];
    }
        ...
        returns (bool)
    {
        ...
        _allowedLBD[from][msg.sender] = _allowedLBD[from][msg.sender].sub(value);
        ...
        returns (bool)
    {
        ...
        _allowedLBD[msg.sender][spender] = value;
        ...
        returns (bool)
    {
        ...
        emit Approval(msg.sender, spender, _allowedLBD[msg.sender][spender]);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        uint256 oldValue = _allowedLBD[msg.sender][spender];
        if (subtractedValue >= oldValue) {
            _allowedLBD[msg.sender][spender] = 0;
        } else {
            _allowedLBD[msg.sender][spender] = oldValue.sub(subtractedValue);
        }
        emit Approval(msg.sender, spender, _allowedLBD[msg.sender][spender]);
        return true;
    }
